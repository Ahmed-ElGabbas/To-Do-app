...
**Realtime coupling (R8):** the realtime layer adds **no tables** — sockets are stateless except for the in-memory presence registry and room membership. It couples to this model in one direction: the `sessions.revoked` domain event (emitted on `logout-all`, password change/reset, which all go through `AuthService.revokeAllSessions` — an `UPDATE refresh_tokens ... SET is_revoked = true`) is consumed by `RealtimeEventConsumer`, which force-disconnects the user's live sockets with an `auth_error`. The client's one-try refresh then hits the just-revoked token rows and the session ends everywhere. There is no presence/user-status column anywhere in the schema; presence is derived purely from live socket counts.
