...
**Status:** Documents the API exactly as it exists in the current codebase — 22 controllers, verified by direct listing (`find src/modules -name "*.controller.ts"`), covering **88 endpoints** (counted directly, not estimated). This is not a redesign proposal; every route, DTO shape, and status code below is transcribed from real source, not invented.
