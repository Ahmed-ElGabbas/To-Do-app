...
- **SHIPPED (R8, 2026-08-15).** The §2.2 alternative is built: `AuthService.revokeAllSessions` (the single path behind `logout-all`, password change/reset) now publishes a new domain event `SESSIONS_REVOKED = 'sessions.revoked'` (enum added to `src/infrastructure/events/task-event.ts`; both other consumers' switches have `default: break`, so it is additive and safe). `RealtimeEventConsumer.onSessionsRevoked` emits `auth_error { code: 'SESSION_REVOKED' }` to every socket in `user:<id>` and calls `disconnect(true)`. Client-side needs no change: the existing `auth_error` path tries one refresh, which fails because the refresh tokens were just revoked, and `onSessionExpired` signs the user out everywhere.
...
> **Flagged follow-up (R8 decision, 2026-08-15):** the project owner chose to **defer Redis-backed presence** — it stays on the in-memory registry behind the `PresenceRegistry` interface (single-instance correct, graceful degradation across instances, per Section 2.4). Not a design gap: the migration path above is unchanged and swap-contained. Only a post-R8, multi-instance trigger would build it.
...
> **SHIPPED (R8, 2026-08-15).** `RealtimeGateway.verifyAppCheck` (called after JWT verification in `handleConnection`, before packet middleware registration) implements the above exactly: skip when `!firebase.isConfigured()` (no log); trim/validate the `appCheckToken` from `client.handshake.auth`; monitor mode logs the pass/reject/missing cases with the user id (`realtime_app_check_pass` includes the verified `appId`) but always proceeds; when `appCheck.enforce` is true, a missing token → `auth_error { code: 'APP_CHECK_FAILED', message: 'App Check token is required' }` + `disconnect(true)`, an invalid token → `auth_error { code: 'APP_CHECK_FAILED', message: 'App Check verification failed' }` + `disconnect(true)`. `RealtimeModule` imports `FirebaseModule` (not global, no cycle) to reach `FirebaseAdminService`. Flutter sends the attestation as `auth.appCheckToken` fetched from the existing `AppCheckService` facade in `RealtimeService.connect()`/`_refreshAndRetry` (null-safe like the HTTP header — omitted when App Check is inactive or the fetch fails). Enforce mode uses the identical `appCheck.enforce` boolean as HTTP — no second switch.
...
- **R8 — Security polish & docs.** ✅ **SHIPPED (2026-08-15).** Handshake App Check (monitor-mode only, §11.2), session-revocation disconnect (§2.2 alternative), final full-suite pass on both sides, reference docs + plan status updated. **Redis-backed presence deliberately deferred** (§5.3) — owner decision, documented as a flagged follow-up. Backend: `RealtimeGateway` verifies `auth.appCheckToken` via `FirebaseAdminService.getAppCheck().verifyToken()` (skip when unconfigured; log `realtime_app_check_pass`/`realtime_app_check_reject`/`realtime_app_check_missing`; enforce via the same `appCheck.enforce` boolean as HTTP — missing/invalid token → `auth_error { code: 'APP_CHECK_FAILED' }` + `disconnect(true)`); `RealtimeModule` imports `FirebaseModule`; new domain event `SESSIONS_REVOKED` published by `AuthService.revokeAllSessions` (covers `logout-all`, password change/reset) and consumed by `RealtimeEventConsumer` (`auth_error { code: 'SESSION_REVOKED' }` + `disconnect(true)` to the user's sockets; no Flutter change needed — the existing refresh-once-then-sign-out path handles it). Flutter: `RealtimeService` adds the App Check attestation to the handshake `auth` (null-safe via `AppCheckService`, omitted when inactive/failed). Verified: backend `nest build` clean, `npm run lint` 0 errors, `npm test` 385/385, `test:integration` 46/46 (9 new unit cases: 6 gateway App Check, 1 logoutAll publish, 2 consumer sessions.revoked); Flutter `flutter analyze` 0 issues, `flutter test` 223/223 (2 new App Check handshake tests). Reference docs updated: `rest-api-reference.md` §9 (realtime transport), `database-architecture-reference.md` §6 (realtime/session coupling), `firebase-integration-plan.md` §8 (realtime App Check extension).
